...
export const updateGround = async (req, res) => {
    try {
        ...
        // Ensure the images field is an array (if applicable)
        if (groundData.images && !Array.isArray(groundData.images)) {
            return res.status(400).send({
                message: "Invalid data for images, expected an array.",
                success: false,
            });
        }
