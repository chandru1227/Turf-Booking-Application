...
export const deleteBooking = async (req, res) => {
    try {
        ...
        // 1. Attempt to delete the booking from the ground's bookings array
        ...
        if (!ground) {
            return res.status(404).send({ success: false, message: 'Booking not found in Ground' });
        }

        console.log("Booking successfully deleted from Ground:", ground);
        ...
        if (!user) {
            return res.status(404).send({ success: false, message: 'Booking not found in User' });
        }

        console.log("Booking successfully deleted from User:", user);

        // 3. Attempt to delete the booking from the Booking collection
        const booking = await Booking.findByIdAndDelete(id);

        if (!booking) {
            return res.status(404).send({ success: false, message: 'Booking not found in Booking collection' });
        }

        console.log("Booking successfully deleted from Booking collection:", booking);

        // Return success message
        res.send({ success: true, message: 'Booking deleted successfully from Ground, User, and Booking collection' });

    } catch (error) {
        console.error("Error during deletion:", error);
        res.status(500).send({ success: false, message: 'Failed to delete booking', error });
    }
};
